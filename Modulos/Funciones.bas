...
Public Function ValidarPorcentaje(Control As Control) As Boolean
    ...
    End If
End Function
Public Function VALIDO_IMPORTE4(mTEXTO As String) As String
    VALIDO_IMPORTE4 = IIf(Trim(mTEXTO) = "", "0,00", Format(mTEXTO, "#,##0.0000"))
End Function
Public Function Valido_Importe2(mTEXTO As String) As String
    Valido_Importe2 = IIf(Trim(mTEXTO) = "", "0,00", Format(mTEXTO, "#,##0.00"))
End Function
Public Function VALIDO_IMPORTE(mTEXTO As String) As String
    VALIDO_IMPORTE = IIf(Trim(mTEXTO) = "", "0,000", Format(mTEXTO, "#,##0.000"))
End Function
...
Public Function SacoAcento(palabra As String) As String
    Dim i As Integer
    For i = 1 To Len(palabra)
       If InStr(1, palabra, "Á") > 0 Or InStr(1, palabra, "É") > 0 Or InStr(1, palabra, "Í") > 0 Or InStr(1, palabra, "Ó") > 0 Or InStr(1, palabra, "Ú") > 0 Or InStr(1, palabra, "Ñ") > 0 Or InStr(1, palabra, "á") > 0 Or InStr(1, palabra, "é") > 0 Or InStr(1, palabra, "í") > 0 Or InStr(1, palabra, "ó") > 0 Or InStr(1, palabra, "ú") > 0 Or InStr(1, palabra, "ñ") > 0 Or InStr(1, palabra, "ü") > 0 Or InStr(1, palabra, "Ü") > 0 Or InStr(1, palabra, "º") Then
        Select Case Mid(palabra, i, 1)
            Case "Á"
                palabra = Replace(palabra, "Á", "A")
            Case "É"
                palabra = Replace(palabra, "É", "E")
            Case "Í"
                palabra = Replace(palabra, "Í", "I")
            Case "Ó"
                palabra = Replace(palabra, "Ó", "O")
            Case "Ú"
                palabra = Replace(palabra, "Ú", "U")
            Case "Ñ"
                palabra = Replace(palabra, "Ñ", "N")
            Case "á"
                palabra = Replace(palabra, "á", "a")
            Case "é"
                palabra = Replace(palabra, "é", "e")
            ...
            Case "ñ"
                palabra = Replace(palabra, "ñ", "n")
            Case "ü"
                palabra = Replace(palabra, "ü", "u")
            Case "Ü"
                palabra = Replace(palabra, "Ü", "U")
            Case "º"
                palabra = Replace(palabra, "º", "")
        End Select
      End If
    Next i
    SacoAcento = palabra
