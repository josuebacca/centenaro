...
Begin VB.Form frmListadoCantidadesVendidas 
   ...
   EndProperty
   Icon            =   "frmListadoCantidadesVendidas.frx":0000
   KeyPreview      =   -1  'True
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MinButton       =   0   'False
   ...
   Begin VB.Frame fraImpresion 
      ...
      Begin VB.PictureBox picSalida 
         ...
         Index           =   2
         Left            =   135
         Picture         =   "frmListadoCantidadesVendidas.frx":0442
         ScaleHeight     =   240
         ScaleWidth      =   240
         TabIndex        =   18
         Top             =   315
      ...
      Begin VB.PictureBox picSalida 
         ...
         Index           =   1
         Left            =   135
         Picture         =   "frmListadoCantidadesVendidas.frx":0544
         ScaleHeight     =   240
         ScaleWidth      =   240
         TabIndex        =   17
         Top             =   315
      ...
      Begin VB.PictureBox picSalida 
         ...
         Index           =   0
         Left            =   135
         Picture         =   "frmListadoCantidadesVendidas.frx":0646
         ScaleHeight     =   240
         ScaleWidth      =   240
         TabIndex        =   16
         Top             =   315
      ...
      Begin VB.ComboBox cboDestino 
         Height          =   315
         ItemData        =   "frmListadoCantidadesVendidas.frx":0748
         Left            =   450
         List            =   "frmListadoCantidadesVendidas.frx":0755
         Style           =   2  'Dropdown List
         TabIndex        =   7
         Top             =   270
         Width           =   1635
...
Private Sub cboListar_Click()
    If frmListadoCantidadesVendidas.Visible = True Then
...
Private Function cantVend()
   ...
   'facturas A
    ...
    If txtFADesde.Text <> "" Then
        sql = sql & " AND FC.TCO_CODIGO=1 AND FC.FCL_NUMERO >= " & XN(txtFADesde.Text)
    End If
    If txtFAHasta.Text <> "" Then
        sql = sql & " AND FC.TCO_CODIGO=1 AND FC.FCL_NUMERO <= " & XN(txtFAHasta.Text)
    End If
    
    sql = sql & " GROUP BY DF.PTO_CODIGO,P.PTO_DESCRI"
    'uno con fact B
    sql = sql & " UNION "
    sql = sql & "SELECT DF.PTO_CODIGO,P.PTO_DESCRI,SUM(DF.DFC_CANTIDAD)AS CANTI,SUM(DF.DFC_CANTIDAD*DF.DFC_PRECIO) AS MONTO"
    sql = sql & " FROM FACTURA_CLIENTE FC, DETALLE_FACTURA_CLIENTE DF, PRODUCTO P"
    sql = sql & " WHERE FC.TCO_CODIGO = DF.TCO_CODIGO"
    sql = sql & " AND FC.FCL_SUCURSAL = DF.FCL_SUCURSAL"
    sql = sql & " AND FC.FCL_NUMERO = DF.FCL_NUMERO"
    ...
    If txtFBDesde.Text <> "" Then
        sql = sql & " AND FC.TCO_CODIGO=2 AND FC.FCL_NUMERO >= " & XN(txtFBDesde.Text)
    End If
    If txtFBHasta.Text <> "" Then
        sql = sql & " AND FC.TCO_CODIGO=2 AND FC.FCL_NUMERO <= " & XN(txtFBHasta.Text)
    End If
    sql = sql & " GROUP BY DF.PTO_CODIGO,P.PTO_DESCRI"
