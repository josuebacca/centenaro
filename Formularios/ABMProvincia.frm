...
Begin VB.Form ABMProvincia 
   ...
   Begin VB.ComboBox cboPais 
      Height          =   315
      ItemData        =   "ABMProvincia.frx":000C
      Left            =   1065
      List            =   "ABMProvincia.frx":000E
...
Const cTabla = "PROVINCIA"
Const cCampoID = "PRO_CODIGO"
Const cDesRegistro = "Provincia"
...
Function SetMode(pMode As Integer)
    ...
    Select Case pMode
        Case 1
            ...
            Me.Caption = "Nueva Provincia.."
            DesacCtrl txtID
            AcCtrl txtDescri
        Case 2
            cmdAceptar.Enabled = False
            Me.Caption = "Editando Provincia..."
            DesacCtrl txtID
            'DesacCtrl txtDescri
        Case 3
            cmdAceptar.Visible = False
            Me.Caption = "Datos de la Provincia..."
            DesacCtrl txtID
            DesacCtrl txtDescri
        Case 4
            cmdAceptar.Enabled = True
            Me.Caption = "Eliminando Provincia..."
            DesacCtrl txtID
            DesacCtrl txtDescri
    End Select
    ...
    If pMode <> 4 Then
        ...
        If txtID.Text = "" Then
            ...
            MsgBox "Falta información." & Chr(13) & _
                             "Ingrese la Identificación del Provincia antes de aceptar.", vbCritical + vbOKOnly, App.Title
            txtID.SetFocus
            Exit Function
        ElseIf txtDescri.Text = "" Then
            Beep
            MsgBox "Falta información." & Chr(13) & _
                             "Ingrese la descripción del Provincia antes de aceptar.", vbCritical + vbOKOnly, App.Title
            txtDescri.SetFocus
            Exit Function
        End If
    End If
    ...
    If Validar(vMode) = True Then
        ...
        Select Case vMode
            Case 1 'nuevo
                ...
                cSQL = cSQL & "     (PAI_CODIGO, PRO_CODIGO ,PRO_DESCRI) "
                cSQL = cSQL & "VALUES "
            ...
            Case 2 'editar
                ...
                cSQL = cSQL & "     PRO_DESCRI = " & XS(txtDescri.Text)
                cSQL = cSQL & " WHERE PRO_CODIGO  = " & XN(txtID.Text)
                cSQL = cSQL & " AND PAI_CODIGO = " & cboPais.ItemData(cboPais.ListIndex)
            
            Case 4 'eliminar
            
                cSQL = "DELETE FROM " & cTabla & " WHERE PRO_CODIGO  = " & XN(txtID.Text)
                cSQL = cSQL & " AND PAI_CODIGO = " & cboPais.ItemData(cboPais.ListIndex)
...
ErrorTran:
    ...
    MsgBox Err.Description, TIT_MSGBOX
...
Private Sub Form_Load()
    ...
    If vMode <> 1 Then
        If vFieldID <> "0" Then
            cSQL = "SELECT * FROM " & cTabla & "  WHERE PRO_CODIGO = " & Mid(vFieldID, 2, Len(vFieldID) - 2)
            cSQL = cSQL & " AND PAI_CODIGO = " & Mid(vFieldID1, 1, 10)
            rec.Open cSQL, DBConn, adOpenStatic, adLockOptimistic
            If (rec.BOF And rec.EOF) = 0 Then
                'si encontró el registro muestro los datos
                Call BuscaCodigoProxItemData(CInt(rec!PAI_CODIGO), cboPais)
                txtID.Text = rec!PRO_CODIGO
