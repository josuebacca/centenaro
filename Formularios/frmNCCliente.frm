...
Begin VB.Form frmNCCliente 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Nota de Credito de Clientes..."
   ClientHeight    =   8010
   ClientLeft      =   300
   ClientTop       =   1365
   ClientWidth     =   10680
   ...
   Begin VB.Frame fraTarjeta 
      ...
      Begin VB.ComboBox cboPlan 
         Height          =   315
         ItemData        =   "frmNCCliente.frx":0000
         Left            =   1665
         List            =   "frmNCCliente.frx":0002
      ...
      Begin VB.ComboBox cboTarjeta 
         Height          =   315
         ItemData        =   "frmNCCliente.frx":0004
         Left            =   1665
         List            =   "frmNCCliente.frx":0006
         Style           =   2  'Dropdown List
         TabIndex        =   67
         Top             =   855
         Width           =   2505
   ...
   Begin VB.Frame fraPagos 
      ...
      Begin VB.ComboBox cboFormaPago 
         Height          =   315
         ItemData        =   "frmNCCliente.frx":0008
         Left            =   1470
         List            =   "frmNCCliente.frx":000A
         Style           =   2  'Dropdown List
         TabIndex        =   87
         Top             =   1470
         Width           =   3330
      ...
      Begin MSFlexGridLib.MSFlexGrid grdPagos 
         ...
         HighLight       =   2
         SelectionMode   =   1
         FormatString    =   $"frmNCCliente.frx":000C
      End
   ...
   Begin TabDlg.SSTab tabDatos 
      ...
      EndProperty
      TabCaption(0)   =   "&Datos"
      TabPicture(0)   =   "frmNCCliente.frx":0012
      ...
      Tab(0).Control(6)=   "fraFac"
      Tab(0).Control(6).Enabled=   0   'False
      Tab(0).ControlCount=   7
      TabCaption(1)   =   "&Buscar"
      TabPicture(1)   =   "frmNCCliente.frx":002E
      Tab(1).ControlEnabled=   0   'False
      Tab(1).Control(0)=   "GrdModulos"
      Tab(1).Control(1)=   "frameBuscar"
      Tab(1).ControlCount=   2
      Begin VB.Frame fraFac 
   ...
   End
End
Attribute VB_Name = "frmNCCliente"
...
Private Function grabar_factura()
        ...
        If rec.EOF = True Then
            ...
            sql = sql & " FCL_NUMEROTXT,FCL_SUCURSALTXT,CLI_CODIGO,FCL_IMPINT,TUR_CODIGO,FCL_HORA,FCL_TASAVIAL)"
            sql = sql & " VALUES ("
...
Private Sub cmdGrabar_Click()
    ...
    If MsgBox("Confirma la impresion de la Factura?", vbQuestion + vbYesNo, TIT_MSGBOX) = vbYes Then
        'hago esto para cuando hagan la primer factura despues de las 00:00
        FechaFactura = Date
...
Private Function ActualizoTotalesFiscales()
    ...
    If txttasavial.Text <> "0,00" Then 'es combustible
        ...
        vNetoFiscal = CDbl(txtNetoFiscal.Text) - CDbl(txttasavial)
        vIVAFiscal = vNetoFiscal * mIVA_1 / 100
        vImpIntFiscal = CDbl(txtIvaFiscal.Text)
...
'    mContPrecio = 100
    ...
    If cboFactura.ItemData(cboFactura.ListIndex) = 1 Then 'factura A
        ...
        mRespuestaFiscal = pf.OpenInvoice("T", "C", "A", "1", "P", "12", "I", mRespo.Text, txtRazSoc.Text, "", "CUIT", txtCuit.Text, "N", Trim(mIngBrutos), Trim(mVendedor), "", "", "", "G")
        'talampaya
...
'                Select Case cboFactura.ItemData(cboFactura.ListIndex)
'                    Case 4 'NOTA CREDITO A
'                        pf.Status ("A")
'                        txtNroFactura.Text = Val(pf.AnswerField_11) + 1
'                    Case 5 'NOTA CREDITO B
'                        pf.Status ("A")
'                        txtNroFactura.Text = Val(pf.AnswerField_12) + 1
'                End Select
'
'If cboFactura.ItemData(cboFactura.ListIndex) = 4 Then 'NOTA CREDITO A
...
'End If
'
'If cboFactura.ItemData(cboFactura.ListIndex) = 5 Then 'NOTA CREDITO B
...
'        If mRespuestaFiscal = False Then Exit Sub
'    End If
'    If cboFactura.ItemData(cboFactura.ListIndex) = 4 Then 'NOTA CREDITO A Then
'        mRespuestaFiscal = pf.GetInvoiceSubtotal("P", "SUBTOTAL")
'        If mRespuestaFiscal = False Then Exit Sub
'    End If
'    If cboFactura.ItemData(cboFactura.ListIndex) = 5 Then 'NOTA CREDITO B Then
'        mRespuestaFiscal = pf.GetInvoiceSubtotal("P", "SUBTOTAL")
'        If mRespuestaFiscal = False Then Exit Sub
'
''CIERRO COMPROBANTE
'
'    If cboFactura.ItemData(cboFactura.ListIndex) = 4 Then 'nota de credito A
'        'pf.GetInvoiceSubtotal "P", "SUBTOTAL"
'        txtTotalFiscal.Text = Round(CDbl(Chk0(pf.AnswerField_5)) / 100, 2)
...
'        If mRespuestaFiscal = False Then Exit Sub
'    End If
'    If cboFactura.ItemData(cboFactura.ListIndex) = 5 Then 'nota de credito B
'        'pf.GetInvoiceSubtotal "P", "SUBTOTAL"
'        txtTotalFiscal.Text = Round(CDbl(Chk0(pf.AnswerField_5)) / 100, 2)
'        txtIvaFiscal.Text = Round(CDbl(Chk0(pf.AnswerField_6)) / 100, 2)
'        txtNetoFiscal.Text = Round(CDbl(Chk0(pf.AnswerField_10)) / 100, 2)
...
Private Sub CmdNuevo_Click()
   ...
   txtTotal.Text = "0,000"
   txtPorcentajeIva.Text = Format(mIVA_1, "0.000")
...
'    cboCondicion_LostFocus
    ...
    txtPorcentajeIva.Text = Format(CStr(mIVA_1), "0.000")
...
'
'                ImpInterno = Valido_Importe(rec!DFC_CANTIDAD * rec!DFC_IMP)
'                ImpSubtotal = rec!FCL_TOTAL - CDbl(ImpInterno)
'                ImpSubtotal = Valido_Importe((ImpSubtotal) / 1.21)
'                ImpIVA = Valido_Importe(rec!FCL_TOTAL - CDbl(ImpInterno) - CDbl(ImpSubtotal))
'
...
Private Sub LlenarComboFactura()
    ...
    If rec.EOF = False Then
        Do While rec.EOF = False
            cboFac.AddItem rec!TCO_DESCRI
            cboFac.ItemData(cboFac.NewIndex) = rec!TCO_CODIGO
...
Private Sub LlenarComboNC()
    sql = "SELECT * FROM TIPO_COMPROBANTE"
    sql = sql & " WHERE TCO_DESCRI LIKE 'NOTA DE CREDITO%'"
    sql = sql & " ORDER BY TCO_DESCRI ASC"
    rec.Open sql, DBConn, adOpenStatic, adLockOptimistic
    If rec.EOF = False Then
...
Private Sub GrdModulos_dblClick()
    ...
    If GrdModulos.Rows > 1 Then
        ...
        BuscoFactura GrdModulos.TextMatrix(GrdModulos.RowSel, 7), Left(GrdModulos.TextMatrix(GrdModulos.RowSel, 1), 4), Right(GrdModulos.TextMatrix(GrdModulos.RowSel, 1), 8)
...
Public Function BuscoFactura(FacTipo As Integer, FacSuc As Integer, FacNro As Integer)
        CmdNuevo_Click
        cmdGrabar.Enabled = False
        mBuscador = True
        tabDatos.Tab = 0
        
        Call BuscaCodigoProxItemData(FacTipo, cboFac)
        txtsucfac.Text = FacSuc
        txtnrofac.Text = FacNro
        ...
        sql = sql & " WHERE DFC.FCL_NUMERO=" & FacNro
        sql = sql & " AND DFC.FCL_SUCURSAL=" & FacSuc
        sql = sql & " AND DFC.TCO_CODIGO=" & FacTipo
        ...
        If Rec1.EOF = False Then
            ...
            Do While Rec1.EOF = False
                ...
                grdGrilla.TextMatrix(i, 6) = Valido_Importe(Chk0(Rec1!DFC_IVA))
                grdGrilla.TextMatrix(i, 7) = Valido_Importe(Chk0(Rec1!DFC_IMP))
                grdGrilla.TextMatrix(i, 8) = Format(Chk0(Rec1!DFC_MONIMP), "0.000")
                grdGrilla.TextMatrix(i, 9) = Format(Chk0(Rec1!DFC_MONIVA), "0.000")
                grdGrilla.TextMatrix(i, 11) = Format(Chk0(Rec1!DFC_TasaVial), "0.000")
                grdGrilla.TextMatrix(i, 12) = Format(Chk0(Rec1!DFC_TOTALTVIAL), "0.000")
                ...
                If Rec1!TCO_CODIGO = 1 Then
                    ...
                    grdGrilla.TextMatrix(i, 4) = Format(mimpneto, "0.000")
                    
                Else
                    grdGrilla.TextMatrix(i, 4) = Format(CDbl(grdGrilla.TextMatrix(i, 2)) * CDbl(grdGrilla.TextMatrix(i, 3)), "0.000")
                End If
                'En esta columna auxiliar pongo el neto sin iva para q se guarde en la BD cuando es Fac B tmb
                grdGrilla.TextMatrix(i, 10) = Format(mimpneto, "0.000")
                i = i + 1
                Rec1.MoveNext
            Loop
        End If
        ...
        sql = sql & " WHERE FCL_NUMERO=" & FacNro
        sql = sql & " AND FCL_SUCURSAL=" & FacSuc
        sql = sql & " AND TCO_CODIGO=" & FacTipo
        
        Rec1.Open sql, DBConn, adOpenStatic, adLockOptimistic
        If Rec1.EOF = False Then
            'encabezado factura
            fechafac.Text = Rec1!FCL_FECHA
            Call BuscoEstado(CInt(Chk0(Rec1!EST_CODIGO)), lblEstadoFactura)
            txtcodCli.Text = Chk0(Rec1!CLI_CODIGO)
            txtCodCli_LostFocus
            txtObservaciones.Text = ChkNull(Rec1!FCL_OBSERVACION)
            
            'CONDICION VENTA
            Call BuscaCodigoProxItemData(Chk0(Rec1!FPG_CODIGO), cboCondicion)
            Call BuscaCodigoProxItemData(Chk0(Rec1!VEN_CODIGO), cboVendedor)
            Call BuscaCodigoProxItemData(Chk0(Rec1!TUR_CODIGO), cboTurno)
            If Rec1!TCO_CODIGO = 1 Then 'FAC A
                txtTotal.Text = Chk0(Rec1!FCL_TOTAL)
                txtTotal.Text = Valido_Importe(txtTotal.Text)
                
                txtsubtotal1.Text = Chk0(Rec1!FCL_SUBTOTAL)
                txtsubtotal1.Text = Valido_Importe(txtsubtotal1.Text)
                
                txtimpuesto.Text = Chk0(Rec1!FCL_IMPINT)
                txtimpuesto.Text = Valido_Importe(txtimpuesto.Text)
                
                txtSubtotal.Text = CDbl(txtsubtotal1.Text) + CDbl(txtimpuesto.Text)
                txtSubtotal.Text = Valido_Importe(txtSubtotal.Text)
                ...
                txttasavial.Text = Valido_Importe(txttasavial.Text)
                'OCULTOS
                txtsubtotal1B.Text = Chk0(Rec1!FCL_SUBTOTAL)
                
                txtimpuestoB.Text = Chk0(Rec1!FCL_IMPINT)
                txtimpuestoB.Text = Valido_Importe(txtimpuestoB.Text)
                
                txtSubtotalB.Text = CDbl(txtsubtotal1B.Text) + CDbl(txtimpuestoB.Text)
                txtSubtotalB.Text = Valido_Importe(txtSubtotalB.Text)
            ...
            Else 'FAC B
                ...
                'OCULTOS
                txtsubtotal1B.Text = Chk0(Rec1!FCL_SUBTOTAL)
                txtsubtotal1B.Text = Valido_Importe(txtsubtotal1B.Text)
...
'                    txtimpuesto.Text = "0.000"
'                    txtTotal.Text = "0,000"
'                    txtPorcentajeIva.Text = Format(mIVAi, "0.000")
'                    txtImporteIva.Text = "0,000"
'                End If
            End If
            ...
            Case 0 'CODIGO
                ...
                If Trim(txtEdit) <> "" Then
                    ...
                    If rec.EOF = False Then
                        ...
                        If Ltipo_fac.Caption = "B" Then
                            mPrecio = Valido_Importe(Chk0(rec!PTO_PRECTO))
                        Else
                            grdGrilla.TextMatrix(grdGrilla.RowSel, 6) = Format(Chk0(mIVA_1), "0.000")
                            
                            mPrecio = Valido_Importe(Chk0(rec!PTO_PRECTO))
                            grdGrilla.TextMatrix(grdGrilla.RowSel, 10) = Valido_Importe(CStr(mPrecio))
                        End If
                        ...
                        grdGrilla.TextMatrix(grdGrilla.RowSel, 7) = Format(Chk0(rec!PTO_IVA), "0.000") ' ALICUOTA IMPUESTO INTERNO
                        grdGrilla.TextMatrix(grdGrilla.RowSel, 11) = Format(Chk0(rec!PTO_TASAVIAL), "0.000") ' ALICUOTA IMPUESTO INTERNO
                        'calculo tasa vial
                        grdGrilla.TextMatrix(grdGrilla.RowSel, 12) = Format(CStr(CDbl(grdGrilla.TextMatrix(grdGrilla.RowSel, 2)) * CDbl(grdGrilla.TextMatrix(grdGrilla.RowSel, 11))), "0.000")
                        ...
                        txtsubtotal1.Text = Valido_Importe(CStr(SumaSUBTotal))
                        txtTotal.Text = Format(CStr(SumaTotal), "0.00")
                        'txtPorcentajeIva_LostFocus
                        Sumatorias
                        mFoco = True
...
'                            End If
                            ...
                            txtsubtotal1.Text = Valido_Importe(CStr(SumaSUBTotal))
                            txtSubtotal.Text = Valido_Importe(CStr(SumaSUBTotal))
                            txtTotal.Text = Format(CStr(SumaTotal), "0.00")
                            'txtPorcentajeIva_LostFocus
                            Sumatorias
                            grdGrilla.Col = 1
                        ...
                        Else
                            ...
                            'mValIVA = Format(Chk0(rec!LIS_IVA), "0.000")
...
'                    If Trim(txtEdit) <> "" Then
                        ...
                        grdGrilla.TextMatrix(grdGrilla.RowSel, 10) = Valido_Importe(CStr(CDbl(grdGrilla.TextMatrix(grdGrilla.RowSel, 2)) * CDbl(grdGrilla.TextMatrix(grdGrilla.RowSel, 3))))
'                    End If
'                End If
                ...
                If Ltipo_fac.Caption = "B" Then
                    ...
                    txtTotal.Text = Format(CStr(SumaTotal), "0.00")
                    txtsubtotal1.Text = Valido_Importe(CStr(SumaSUBTotal))
                    txtSubtotal = Format(CStr(SumaSUBTotal), "0.000")
                Else
                    'txtSubtotal.Text = Valido_Importe(CStr(SumaSUBTotal))
...
'                    txtEdit.Text = Format(mimpneto, "0.000")
'                    grdGrilla.TextMatrix(grdGrilla.RowSel, 4) = Format(txtEdit.Text, "0.000")
'                    grdGrilla.TextMatrix(grdGrilla.RowSel, 10) = Format(txtEdit.Text, "0.000")
'                    txtSubtotal.Text = Format(CStr(SumaSUBTotal), "0.00")
                ...
                txtTotal.Text = Format(CStr(SumaTotal), "0.000")
                'txtPorcentajeIva_LostFocus
                
            Case 3 'PRECIO
                
                If Trim(txtEdit) = "" Then grdGrilla.Text = "0,000"
                txtEdit.Text = Format(txtEdit.Text, "0.000")
                grdGrilla.TextMatrix(grdGrilla.RowSel, 3) = Format(txtEdit.Text, "0.000")
                ...
                If grdGrilla.TextMatrix(grdGrilla.RowSel, 0) <> "" Then
                    If Trim(txtEdit) <> "" Then
                        grdGrilla.TextMatrix(grdGrilla.RowSel, 4) = Format(CDbl(grdGrilla.TextMatrix(grdGrilla.RowSel, 2)) * CDbl(grdGrilla.TextMatrix(grdGrilla.RowSel, 3)), "0.000")
                        grdGrilla.TextMatrix(grdGrilla.RowSel, 10) = Format(CDbl(grdGrilla.TextMatrix(grdGrilla.RowSel, 2)) * CDbl(grdGrilla.TextMatrix(grdGrilla.RowSel, 3)), "0.000")
                        'calculo tasa vial
                        grdGrilla.TextMatrix(grdGrilla.RowSel, 12) = Format(CStr(CDbl(grdGrilla.TextMatrix(grdGrilla.RowSel, 2)) * CDbl(grdGrilla.TextMatrix(grdGrilla.RowSel, 11))), "0.000")
                    End If
                End If
             ...
             Case 4 'importe
                ...
                If grdGrilla.TextMatrix(grdGrilla.RowSel, 0) <> "" Then
                    If Trim(txtEdit) <> "" Then
                        grdGrilla.TextMatrix(grdGrilla.RowSel, 2) = Format(CStr(CDbl(grdGrilla.TextMatrix(grdGrilla.RowSel, 4)) / CDbl(grdGrilla.TextMatrix(grdGrilla.RowSel, 3))), "0.000")
                        'calculo tasa vial
                        grdGrilla.TextMatrix(grdGrilla.RowSel, 12) = Format(CStr(CDbl(grdGrilla.TextMatrix(grdGrilla.RowSel, 2)) * CDbl(grdGrilla.TextMatrix(grdGrilla.RowSel, 11))), "0.000")
                    End If
                End If
...
'        pImpue = 1
        ...
        If grdGrilla.TextMatrix(grdGrilla.RowSel, 0) = 1 Or grdGrilla.TextMatrix(grdGrilla.RowSel, 0) = 2 Or grdGrilla.TextMatrix(grdGrilla.RowSel, 0) = 4 Then
            ...
            If Ltipo_fac = "A" Then
                grdGrilla.TextMatrix(grdGrilla.RowSel, 4) = Format(subtotal, "0.000")
            End If
            grdGrilla.TextMatrix(grdGrilla.RowSel, 10) = Format(subtotal, "0.000")
            ImpIVA = subtotal * (mIVA_1 / 100)
            grdGrilla.TextMatrix(grdGrilla.RowSel, 9) = Format(ImpIVA, "0.000")
        Else
        'GASOIL
            subtotal = grdGrilla.TextMatrix(grdGrilla.RowSel, 2) * pPrecio - grdGrilla.TextMatrix(grdGrilla.RowSel, 8)
            ...
            grdGrilla.TextMatrix(grdGrilla.RowSel, 8) = Format(IMPUESTO + mTasa, "0.000")
            grdGrilla.TextMatrix(grdGrilla.RowSel, 9) = Format(ImpIVA, "0.000")
        End If
...
Private Function Sumatorias()
    ...
    txtsubtotal1B.Text = Format(VsubtotalB, "0.000")
    txtimpuestoB.Text = Format(Vimpuesto, "0.000")
    txtSubtotalB.Text = Format(VsubtotalB + Vimpuesto, "0.000")
    txtImporteIvaB.Text = Format(Vimpiva, "0.000")
        
    
    txtsubtotal1.Text = Format(vSubtotal, "0.000")
    If Ltipo_fac = "B" Then
        Vimpuesto = 0
        Vimpiva = 0
    End If
    txtimpuesto.Text = Format(Vimpuesto, "0.000")
    txtSubtotal.Text = Format(vSubtotal + Vimpuesto, "0.000")
    txtImporteIva.Text = Format(Vimpiva, "0.000")
    txttasavial.Text = Format(vTasaVial, "0.000")
...
Private Function SumaSUBTotal() As Double
    'If Ltipo_fac = "A" Then
        ...
        For i = 1 To grdGrilla.Rows - 1
            ...
            End If
        Next
        SumaSUBTotal = Valido_Importe(CStr(VTotal))
    'End If
End Function
...
Private Sub txtnrofac_Change()
    txtnrofac.Text = Format(txtnrofac.Text, "00000000")
End Sub

Private Sub txtnrofac_GotFocus()
    SelecTexto txtnrofac
End Sub
...
Private Sub txtnrofac_LostFocus()
    If txtnrofac.Text <> "" Then
        BuscoFactura cboFac.ItemData(cboFac.ListIndex), txtsucfac, txtnrofac
    End If
    txtnrofac.Text = Format(txtnrofac.Text, "00000000")
End Sub
    ...
    With B
        ...
        If .ResultFields.Count > 0 Then
            ...
            Else
                ...
                mIVA_1 = Format(Chk0(.ResultFields(4)), "0.000")
                If Ltipo_fac.Caption = "B" Then
...
Private Sub txtsucfac_Change()
    txtsucfac.Text = Format(txtsucfac.Text, "0000")
End Sub

Private Sub txtsucfac_GotFocus()
    SelecTexto txtsucfac
End Sub

Private Sub txtsucfac_KeyPress(KeyAscii As Integer)
    KeyAscii = CarNumeroEntero(KeyAscii)
End Sub

Private Sub txtsucfac_LostFocus()
    If txtsucfac.Text = "" Then
        txtsucfac.Text = Format(Sucursal, "0000")
    Else
        txtsucfac.Text = Format(txtsucfac.Text, "0000")
    End If
End Sub
